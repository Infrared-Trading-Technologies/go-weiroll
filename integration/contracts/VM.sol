 ...
 * @notice Abstract base contract for executing weiroll command sequences
            ...
            if (flags & FLAG_CT_MASK == FLAG_CT_DELEGATECALL) {
                ...
                (success, outdata) = address(uint160(uint256(command))).call{
                    ...
                    state.buildInputs(
                        bytes4(command),
                        bytes32(
                            uint256(indices << 8) | CommandBuilder.IDX_END_OF_ARGS
                        )
                    )
                );
            } else {
...
/**
 * @title WeirollVM
 * @notice Concrete implementation of the Weiroll VM
 */
contract WeirollVM is VM {
    ...
    // Allow receiving ETH (needed for WETH withdraw, etc.)
